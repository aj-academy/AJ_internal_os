  ...
  if (selectedTab === "summary") {
    ...
    if (firstRes.error && errMsg.includes("manager_remarks")) {
      ...
      if (secondRes.error) {
        ...
        summarySchemaNotice =
          "Your database is missing work_summaries.manager_remarks, so summaries still load without admin remarks. Run AJ_Academy_SB/attendance_module.sql in Supabase, then refresh.";
      }
    } else {
      ...
      <section className="space-y-6 rounded-[24px] border border-[#d4deea] bg-white p-4 sm:p-6 shadow-[0_22px_40px_rgba(30,64,175,0.09)] lg:p-8">
        ...
        {summaryFetchError ? (
          <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">
            ...
            <code className="rounded bg-white/80 px-1">AJ_Academy_SB/attendance_module.sql</code> in Supabase so tables and RLS match the app (one file for the attendance domain).
          </div>
        ) : null}
