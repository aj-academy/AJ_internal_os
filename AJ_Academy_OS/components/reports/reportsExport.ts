...
/** Provenance shown on an export so a downloaded file can be traced later. */
...
function metaPairs(meta: ExportMeta): [string, string][] {
  ...
  if (meta.report) pairs.push(["Report", meta.report]);
  if (meta.dateRange) pairs.push(["Date range", meta.dateRange]);
  if (meta.filters) pairs.push(["Filters", meta.filters]);
  if (meta.generatedBy) pairs.push(["Generated by", meta.generatedBy]);
  pairs.push(["Generated at", meta.generatedAt || new Date().toLocaleString("en-IN")]);
  return pairs;
}

export function exportRowsAsCsv(filename: string, rows: ExportRow[], meta?: ExportMeta) {
  ...
  let csv = buildCsv(csvHeaders, tableRows);
  if (meta) {
    // Provenance pairs, then a blank line so the data block below stays a
    // contiguous table that spreadsheets can still parse.
    csv = `${buildCsv(["Field", "Value"], metaPairs(meta))}\n\n${csv}`;
  }
...
export async function exportMultiSheetExcel(
  ...
  if (meta) {
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([["Field", "Value"], ...metaPairs(meta)]),
      "Report Info",
    );
  }
