...
**Late check-in email:** After a successful punch-in, `POST /api/notifications/attendance-late` evaluates office start + grace from `attendance_policies` using **IST (`Asia/Kolkata`)**. If late, it sends a dedicated HR attendance notice once per day via **Zoho/Gmail company SMTP first** (best inbox delivery), then Resend only if `TASK_EMAIL_FROM` is a real domain (not `onboarding@resend.dev`, which spam-filters). Also creates in-app/push `hr_attendance_late`.
...
Run **`college_visits_schema.sql`** after `schema.sql` (requires `is_admin()` / profiles), then **`college_visits_visited_by_patch.sql`** (adds `visited_by` / `visited_by_name` for who visited), then **`college_visits_proposal_patch.sql`** (Proposal Tracker: link + PDF columns and `college-visit-proposals` storage bucket), then **`college_visits_contacts_patch.sql`** (multiple contacts: name / role / alternate phones / email JSON + primary sync), then **`proposals_file_upload_patch.sql`** (unified private `proposals` bucket + file columns on `clients` and `college_visits` for PDF/DOC/DOCX upload on Add/Edit), then **`proposals_multi_file_patch.sql`** (multi-file attachments table for Student/College proposals), then **`college_visit_import_batches.sql`** (Admin → College Visits file import batches, duplicate preview before insert, bulk import API), then **`crm_owner_isolation.sql`**, then **`college_visit_file_visibility_patch.sql`**, then **`college_visit_folder_owner_access_patch.sql`**, then **`crm_delete_fix.sql`**. Adds:

- `/admin/college-visits` and `/employee/college-visits` — **same subsection tabs as Student Master**: Overview, All Colleges, Follow-ups, Pipeline, Converted Colleges, MOU Tracker, **Proposal Tracker**, Activity Timeline (+ Reports / Settings for admin). **Admin sees all employees’ colleges**; **employees see their own/created/task-linked/pinned colleges plus every college in folders they uploaded** (including rows Admin later saved into that file). Employees never list Admin-owned uploads. Share via College Visit tasks without opening another employee’s full CRM.
- **All Colleges (admin + employee):** uploads appear as rows (file name + date + **Uploaded by** name/role). Click a row to open duplicate preview and the full column table; confirm **Import** to save new rows only (duplicates skipped). Bulk import uses `/api/college-visits/import/*` (fast chunked insert). Re-uploading the same file shows duplicate preview before anything is saved. **Import template**, **Import**, and **Export** are on both Admin and Employee All Colleges. Employee imports create folders owned by that Employee (`uploaded_by`); Admin still sees those folders, colleges, and the Employee’s name. Employees never list or import into Admin-owned uploads.
- **Add College save location:** creating an individual college now opens a final location dialog for Admin and for Employee when they are not already inside their own folder. Admins can create a named folder or select any existing upload/manual folder; **All Colleges** remains available as the no-folder option. Employees see the same dialog with **the folders already on their All Colleges list** (plus unbatched All Colleges); a new folder name is created on `POST /api/college-visits` (not a separate import call). Saving from inside an Employee-owned folder still goes into that folder with no extra prompt. Manual folders reuse `college_visit_import_batches`, so `college_visits.import_batch_id` remains the single folder link and no spreadsheet file is fabricated or modified.
- **Employee College Visits:** `/employee/college-visits` reuses the same shared workbench, including Import template / bulk Import / Export. Employee-created colleges and Employee spreadsheet uploads save under the authenticated Employee; Admin continues to see all Admin/Employee records (including Employee folders and unbatched Employee colleges grouped under that Employee’s name). Tables and details show creator name, role, and creation date.
- **Required file privacy patch:** run **`college_visit_file_visibility_patch.sql` before deploying the matching app code**. College proposal uploads derive visibility from the authenticated session: Admin/Super Admin → `admin_only`; Employee → `admin_employee`. Proposal list/sign/upload/remove APIs verify parent-record access, Employees never receive Admin-only metadata or signed URLs, and Admin sees uploader name/role/date/visibility. The private `proposals` bucket remains private.
- The old **public** `college-visit-proposals` URLs and public WhatsApp `task-attachments` require separate object migration/product approval; the new patch does not change their bucket privacy automatically.
...
API (staff session): `GET/POST /api/college-visits`, `PATCH/DELETE /api/college-visits/[id]`, `GET/POST /api/college-visits/[id]/activities`. GET returns **all rows for admin**, else the signed-in employee’s owned/created/task-linked/pinned rows **and every college in folders they uploaded**. List GET uses the service role after session checks (avoids per-row `task_links_college` RLS), redacts Employee file URL fields, and does not overlay every proposal file on the list. PATCH and College proposal upload/list/sign/remove use the same folder-owner check, so Save + proposal files work on every row in that Employee’s file.
...
- Admin → **Student Management → Student Directory** — all students with allocated mentors
...
- Mentor → **My Students** — students allotted to that mentor (primary/secondary/project filters)
