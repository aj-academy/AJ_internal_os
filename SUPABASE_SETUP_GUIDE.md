...
- **Add College save location:** If staff are **inside an opened folder**, Save puts the college in that folder with no extra dialog (Admin and Employee). If they add a college from the folder **list** or another tab, a location dialog always asks: **create a new folder**, or **pick an existing named folder** from All Colleges (same files they already see), or save with no folder. Employees create a new folder name on `POST /api/college-visits`. Manual folders reuse `college_visit_import_batches`.
