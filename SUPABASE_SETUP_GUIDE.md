...
- **Add College save location:** creating an individual college now opens a final location dialog. Admins can create a named folder or select any existing upload/manual folder; **All Colleges** remains available as the no-folder option. Manual folders reuse `college_visit_import_batches` through `POST /api/college-visits/import`, so `college_visits.import_batch_id` remains the single folder link and no spreadsheet file is fabricated or modified.
- **Employee College Visits:** `/employee/college-visits` reuses the same shared workbench, including Import template / bulk Import / Export. Employee-created colleges and Employee spreadsheet uploads save under the authenticated Employee; Admin continues to see all Admin/Employee records. Tables and details show creator name, role, and creation date.
