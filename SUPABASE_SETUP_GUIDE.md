...
- **All Colleges (admin + employee):** uploads appear as rows (file name + date). Click a row to open duplicate preview and the full column table; confirm **Import** to save new rows only (duplicates skipped). Bulk import uses `/api/college-visits/import/*` (fast chunked insert). Re-uploading the same file shows duplicate preview before anything is saved. **Import template**, **Import**, and **Export** are on both Admin and Employee All Colleges. Employee imports create folders owned by that Employee (`uploaded_by`); Admin still sees those folders and colleges. Employees never list or import into Admin-owned uploads.
