...
- **All Colleges (admin):** uploads appear as rows (file name + date + **Uploaded by** name/role). Click a row to open duplicate preview and the full column table; confirm **Import** to save new rows only (duplicates skipped). Bulk import uses `/api/college-visits/import/*`. Re-uploading the same file shows duplicate preview before anything is saved. **Import template**, **Import**, and **Export** are on Admin All Colleges. Employee imports still create folders owned by that Employee (`uploaded_by`); Admin still sees those folders, colleges, and the Employee’s name.
- **All Colleges (employee):** a single college table of every college that Employee can access (own/created/task-linked/pinned/folder rows). No folder-first screen and no save-location dialog — **Add College** saves immediately. Existing college rows are not rewritten. **Import template / Import / Export** remain available; after Import save they return to the same table.
- **Add College save location (admin only):** If Admin is **inside an opened folder**, Save puts the college in that folder with no extra dialog. If they add a college from the folder **list** or another tab, a location dialog asks: **create a new folder**, **pick an existing named folder**, or save with no folder. Manual folders reuse `college_visit_import_batches`.
- **Employee College Visits:** `/employee/college-visits` reuses the same tabs (without Reports / Settings). Employee-created colleges save under the authenticated Employee; Admin continues to see all Admin/Employee records. Tables and details show creator name, role, and creation date.
