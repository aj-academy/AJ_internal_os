...
- **All Colleges (admin):** a single college table of every college (Admin + Employee). **Created By** shows who added the college (filter by Admin/Employee or a person). **Location** header filter lists every non-empty location already in the loaded rows; a newly saved location appears in the dropdown after refresh. No folder-first screen and no save-location dialog — **Add College** saves immediately. Existing college rows are not rewritten. **Import template / Import / Export** remain available; after Import save they return to the same table. Employee spreadsheet uploads still create folders in the database so history is kept; Admin just sees the colleges in one table.
- **All Colleges (employee):** a single college table of every college that Employee can access (own/created/task-linked/pinned/folder rows). No folder-first screen and no save-location dialog — **Add College** saves immediately. Existing college rows are not rewritten. **Import template / Import / Export** remain available; after Import save they return to the same table. Location header filter uses the same live row values as Admin.
