...
- `/admin/college-visits` and `/employee/college-visits` — **same subsection tabs as Student Master**: Overview, All Colleges, Follow-ups, Pipeline, Converted Colleges, MOU Tracker, **Proposal Tracker**, Activity Timeline (+ Reports / Settings for admin). **Admin and Employee both see every college.** Admin-only proposal files stay hidden from Employees. Share via College Visit tasks still works.
- **All Colleges (admin):** a single college table of every college (Admin + Employee). **Created By** shows who added the college (filter by Admin/Employee or a person). **Location** header filter lists **city names** found in the loaded rows (full street addresses are not added). A newly saved city appears in the dropdown after refresh. No folder-first screen and no save-location dialog — **Add College** saves immediately. Existing college rows are not rewritten. **Import template / Import / Export** remain available; after Import save they return to the same table. Employee spreadsheet uploads still create folders in the database so history is kept; Admin just sees the colleges in one table.
- **All Colleges (employee):** the same full college table as Admin. **Import template / Import / Export** remain available; after Import save they return to the same table. Location header filter uses city names only. Admin-only proposal files stay hidden.
...
- Proposal Tracker / Add·Edit forms upload **PDF, DOC, or DOCX** (max 10 MB) **directly to** the private `proposals` bucket (signed upload URL — the file does not pass through the app server, so Vercel’s ~4.5 MB body limit cannot block it). Legacy URL/PDF fields remain readable. College Visit proposal upload now supports **multiple files**.
