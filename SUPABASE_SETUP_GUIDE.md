...
- **Add College save location:** creating an individual college now opens a final location dialog. Admins can create a named folder or select any existing upload/manual folder; **All Colleges** remains available as the no-folder option. Employees skip that dialog: a new college is saved into that Employee’s own folder (`POST /api/college-visits/import`, then `import_batch_id`), so it appears on Admin All Colleges as a folder with **Uploaded by** that Employee. Manual folders reuse `college_visit_import_batches`, so `college_visits.import_batch_id` remains the single folder link and no spreadsheet file is fabricated or modified.
