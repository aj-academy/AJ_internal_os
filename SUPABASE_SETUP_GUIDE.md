...
- **All Colleges (admin):** uploads appear as rows (file name + date). Click a row to open duplicate preview and the full column table; confirm **Import** to save new rows only (duplicates skipped). Bulk import uses `/api/college-visits/import/*` (fast chunked insert). Re-uploading the same file shows duplicate preview before anything is saved.
...
- **Employee College Visits:** `/employee/college-visits` reuses the same shared workbench. Employee-created colleges save under the authenticated Employee in **All Colleges**; Admin continues to see all Admin/Employee records. Tables and details show creator name, role, and creation date.
