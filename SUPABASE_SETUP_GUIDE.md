...
- **Add College save location:** creating an individual college now opens a final location dialog. Admins can create a named folder or select any existing upload/manual folder; **All Colleges** remains available as the no-folder option. Employees skip that dialog: `POST /api/college-visits` creates or reuses that Employee’s manual folder (service role after a staff session), so the college appears on Admin All Colleges with **Uploaded by** that Employee. Manual folders reuse `college_visit_import_batches`, so `college_visits.import_batch_id` remains the single folder link and no spreadsheet file is fabricated or modified.
